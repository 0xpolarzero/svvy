...
export const PRODUCT_FEATURES: ProductFeature[] = [
  ...
  },
  {
    ...
    status: "shipped",
    summary:
      "Keeps each workspace tab navigable with pinned active sessions, durable session-level unread dots that appear when assistant turns finish outside the focused pane surface and clear on session-pane focus or explicit mark-read action, a single collapsed-by-default Archived group for non-destructive session hiding, layered sidebar rows where orchestrator session state, handler-thread state, and workflow-run state stay local to their owning rows, session row context menus for mark read or unread, pin, rename, and archive actions, compact running indicators, tone-aware open-pane highlighting, context-budget rails for open orchestrator and handler rows, a sidebar footer that shows the current git branch with a branch icon and opens a local-branch switcher when the workspace is a git repo, compact thread and workflow-run artifact blocks backed by durable artifact records, compact latest Project CI projection near the focused surface or relevant handler thread, and restart restoration for stable Dockview panel bindings, focus, panel-local scroll, display preferences, and inspector selections while deliberately excluding transient UI, composer drafts, transcript selections, and stale live stream state.",
    sourceSpecs: [
      "docs/prd.md",
