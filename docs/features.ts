...
export const PRODUCT_FEATURES: ProductFeature[] = [
  ...
  },
  {
    ...
    status: "in-progress",
    summary:
      "Separates integrated app-chrome workspace tabs, shared durable workspace state, live surface runtimes, and Dockview-backed view-local layout state, using one backend workspace runtime per canonical cwd with explicit `workspaceId` routing for every workspace-scoped request and sync event, never active workspace routing, plus separate workspace-tab/view identity for layout state; opens a real svvy-owned default workspace tab with an `Open Workspace` surface when no user workspace tabs restore; lets `Open Workspace` retarget the current visual tab, `New Tab` create another default workspace tab, and `Open Workspace in New Tab` create a selected user workspace tab; allows opening the same cwd in multiple visual workspace tabs that share the same runtime, session catalog, pi sessions, structured state, prompt queues, handler threads, workflow runs, app logs, and workspace read models while isolating only visual state such as layout slots, opened panels, panel bindings, scroll positions, focused panes, and inspector selections; keeps workspace tabs left-aligned at the start of the main chrome, horizontally scrollable when crowded, draggable for user reordering, durably restored in user-defined order, and paired with compact icon controls plus colored running, unread, waiting, and error count badges shown only above zero with hover context; uses Dockview core for panels, groups, tabs, tab groups, splitters, drag/drop overlays, edge groups, floating groups, popouts, and serialized layout restore inside fixed per-tab layout slots A, B, and C pinned at the far right while svvy stores panel-to-surface bindings and panel-local metadata; keeps empty layout slots muted but selectable, manages live pi surfaces in a shared registry keyed by `surfacePiSessionId`, gives each surface its own prompt lock, model or reasoning lifecycle, pending user message, queued follow-up messages, and surface-owned live assistant stream state, supports explicit open and close semantics, sidebar panel-location indicators, compact thread and workflow-run projections, and lets zero, one, or multiple panels attach to the same streaming surface without duplicating or cancelling the underlying runtime while keeping panel-local scroll independent.",
    sourceSpecs: [
      "docs/prd.md",
