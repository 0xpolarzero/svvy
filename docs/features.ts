...
export const PRODUCT_FEATURES: ProductFeature[] = [
  ...
  },
  {
    ...
    status: "shipped",
    summary:
      "Persists app-wide default-session, dumb-orchestrator, and namer session-agent defaults with provider, model, reasoning, and system prompt as app-global settings; records session mode and prompt selection on created sessions; uses the same configured `namer` for top-level session titles and handler-thread titles derived from delegated objectives; allows handler threads to carry per-thread model, reasoning, and prompt overrides through `thread_start`; exposes focused-surface agent summaries in pane chrome and TanStack Form-backed direct-saving settings inspection/editing for session agents and conventional workflow agents through connected-provider model dropdowns plus selected-model reasoning dropdowns derived from pi's normalized model metadata and runtime thinking controls rather than svvy-owned provider/model special cases, with validation, dirty state, reset, pending, and async error state; and synchronizes `explorer`, `implementer`, and `reviewer` workflow-agent settings to the requested workspace's `.svvy/workflows/components/agents.ts` as a normal saved component asset through explicit `workspaceId` routing.",
    sourceSpecs: [
      "docs/prd.md",
      "docs/specs/structured-session-state.spec.md",
