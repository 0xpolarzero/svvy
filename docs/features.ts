...
export const PRODUCT_FEATURES: ProductFeature[] = [
  ...
  },
  {
    ...
    status: "in-progress",
    summary:
      "Lets a user submit follow-up messages to an already-running orchestrator or handler-thread surface by placing them in a visible FIFO queue owned by the target `surfacePiSessionId`; ordinary queued sends do not steer, interrupt, or create concurrent turns, while a row-level `Steer` action deliberately locks the row and uses pi/Codex-style steering at the next safe boundary; the queue is a typed surface queue where all surfaces accept `user_message` items and the orchestrator also accepts `handler_handoff` items created by blocking `thread.handoff` tool calls; queued items are claimed atomically by one shared queue runner per `surfacePiSessionId`, dispatching rows stay visible as locked in-flight state until accepted as the pending input, remain structured product state until delivered as real pi input, survive panel focus changes and duplicated panels, write user prompt history once at queue time, and stay recoverable across restart, cancellation, restore-to-composer, and pre-accept delivery failure.",
