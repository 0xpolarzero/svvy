...
The desktop shell presents open workspaces as compact tabs inside the app chrome, integrated with the sidebar and workspace control row rather than as a separate top toolbar. Workspace tabs are left-aligned at the start of the main workspace chrome, scroll horizontally when the open tab set exceeds the available space, and can be dragged to reorder them. Workspace tab order is durable workspace-shell state and restores across app restart. A workspace tab is a visual workspace view over one workspace runtime. The canonical workspace runtime and durable workspace state belong to the workspace context, not to the visual tab: the session catalog, path index, app logs, live surface registry, pi sessions, structured state, prompt queues, handler threads, workflow runs, and workspace read models are shared by duplicate tabs for the same canonical cwd. Duplicate same-cwd tabs isolate only view-local state such as active layout slot, Dockview layout, opened panels, panel bindings, panel-local scroll, focused pane, inspector selection, and other visual restore metadata. Opening the app with no restored tabs creates a real svvy-owned default workspace tab whose first focused surface is `Open Workspace`, so normal chat, Context, Logs, command palette, sessions, and layout behavior remain usable before a user chooses a repository. `Open Workspace` retargets the current visual tab to the chosen user workspace, `New Tab` creates another default workspace tab with the `Open Workspace` surface, and `Open Workspace in New Tab` creates a new visual tab for the chosen user workspace. Opening an already-open repository in a new tab creates a separate visual workspace tab for the same cwd instead of focusing the existing tab, without creating an independent workspace runtime, independent session catalog, or isolated durable workspace state.

Each workspace tab summarizes that workspace's session-level running, unread, waiting, and error counts from the shared durable workspace read models for its cwd. Count badges render only when their value is greater than zero, stay in the stable running, unread, waiting, error order, use status color instead of icons, and expose title or tooltip context on hover. Workspace open and close controls are compact icon controls with accessible labels. Workspace-scoped backend requests and renderer sync events carry an explicit `workspaceId` for the shared workspace runtime and, when view-local state is involved, a distinct workspace tab or view id. The backend must not route user work through a process-global active cwd, treat cwd alone as the runtime id, or treat duplicate same-cwd tabs as separate durable workspaces.
...
Each workspace tab has three fixed view-local layout slots: `A`, `B`, and `C`. These are not user-named layouts. The slots render as compact controls pinned at the far right of the same chrome row as the workspace tabs and status controls. Selecting a layout slot swaps the active Dockview layout snapshot for that workspace tab only. Empty slots remain selectable and render muted, not disabled, so the user can start a new layout from scratch. Duplicate same-cwd tabs may have different active slots and layouts while still sharing the same sessions, live surfaces, queues, threads, workflow runs, app logs, and durable workspace read models.
