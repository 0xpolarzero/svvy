...
- [ ] Keep durable unread state session-level with sidebar timestamp dots, focus-to-read clearing, and session row context-menu actions for mark read or unread, pin, rename, and archive; pane unread treatment, when present, reads from the same session metadata.
...
- [ ] Keep open workspaces as left-aligned, horizontally scrollable, draggable app-chrome tabs with durable user-defined tab order, compact icon controls, >0-only colored status count badges, a svvy-owned default workspace runtime when no user workspace tabs restore, an `Open Workspace` panel as the default workspace's first surface, current-tab `Open Workspace`, `New Tab` as a new default workspace tab, and `Open Workspace in New Tab` as picker-backed user workspace tab creation; duplicate same-cwd tabs are separate visual views over the same backend workspace runtime, session catalog, durable workspace state, live surface registry, queues, threads, workflow runs, and app logs, with only view-local layout, opened panels, scroll, focus, and inspector state isolated per tab.
