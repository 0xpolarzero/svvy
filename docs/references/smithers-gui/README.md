...
| **Codex FFI** (`libcodex_ffi.a`) | Rust FFI library powering agent execution. Vendored as a squashed git subtree (`codex/`) and linked at compile time — no runtime install needed. | Build with `codex/codex-rs` |
| **Ghostty** (`GhosttyKit.xcframework`) | Terminal emulator framework for all terminal surfaces. Vendored as a squashed git subtree (`ghostty/`) and linked at compile time — no runtime install needed. | Build the xcframework from `ghostty/` |
